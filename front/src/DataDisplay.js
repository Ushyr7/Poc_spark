...
function EntityTable({ history }) {
  ...
  const handleVulnerabilities = (id) => {
      history.push(`/perimeter/${id}/vulnerability`);
  };
            ...
            {perimeter.length ? (
              perimeter.map(item => (
                <tr key={item.id}>
                  ...
                  <td style={tableDataStyle}><Link to={`/modification-perimetre/${item.id}`}><button className='rounded-button' >Modifier</button></Link>
                    <button className='rounded-button' onClick={() => handleDelete(item.id)}>Supprimer</button>
                    <Link to={`/perimeter/${item.id}/vulnerability`} onClick={() => handleVulnerabilities(item.id)}><br /> <button className="rounded-button">Vulnerabilities</button><br /></Link>
                  </td>
